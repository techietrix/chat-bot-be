...
const clientUrl = "http://192.168.43.225:3000"
const io = socketIo(server, {
  cors: {
    origin: process.env.FRONTEND_URL || clientUrl,
    methods: ["GET", "POST"]
  }
});
...
const SILENCE_THRESHOLD = 0.01;
const SILENCE_DURATION = 1500; // 2 seconds
...
io.on('connection', (socket) => {
  console.log('A user connected: ', socket?.user || 'user not found');
  let audioBuffer = [];
  ...
  async function processAudio() {
    ...
    try {
      ...
      const response = await getResponse(transcript.data.text)
